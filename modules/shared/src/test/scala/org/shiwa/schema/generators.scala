...
import org.shiwa.generators.nesGen
...
import org.shiwa.security.generators._
...
object generators {
  ...
  val addressGen: Gen[Address] =
    ...
    } yield Address(refineV[SHIAddressRefined].unsafeFrom(s"SHI$par$end"))
  ...
  val transactionGen: Gen[SHITransaction] =
    for {
      src <- addressGen
      dst <- addressGen
    ...
    } yield SHITransaction(src, dst, txnAmount, txnFee, txnReference, txnSalt)
  ...
  val signedTransactionGen: Gen[Signed[SHITransaction]] = signedOf(transactionGen)
