...
object TransactionGenerator {
  ...
  def infiniteTransactionStream[F[_]: Async: Random: KryoSerializer: SecurityProvider](
  ...
  ): Stream[F, Signed[SHITransaction]] =
    addressParams.reverse.map {
      case AddressParams(source, key, initialTxRef) =>
        ...
        def tx(lastTxRef: TransactionReference): F[Signed[SHITransaction]] =
          for {
            ...
            tx = SHITransaction(source, destination, amount, fee, lastTxRef, salt)
            signedTx <- tx.sign(key)
          } yield signedTx

        def txStream(lastRef: TransactionReference): Stream[F, Signed[SHITransaction]] =
          for {
    ...
    }.foldLeft[Stream[F, Stream[Pure, Signed[SHITransaction]]]](Stream.constant(Stream.empty)) { (acc, s) =>
