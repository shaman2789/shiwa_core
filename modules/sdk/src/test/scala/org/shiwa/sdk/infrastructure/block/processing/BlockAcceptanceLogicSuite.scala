...
import org.shiwa.sdk.domain.block.processing.{BlockAcceptanceContext, SigningPeerBelowCollateral}
...
object BlockAcceptanceLogicSuite extends MutableIOSuite with Checkers {
  ...
  private val (address1, peer1) = (
    Address("SHI0y4eLqhhXUafeE3mgBstezPTnr8L3tZjAtMWB"),
    PeerId(
      Hex(
  ...
  private val (addressWithoutBalance, peerWithoutBalance) = (
    Address("SHI07tqNLYW8jHU9emXcRTT3CfgCUoumwcLghopd"),
    PeerId(
      Hex(
  ...
  test("accept block with signers with collateral") { implicit sc =>
    forall(dagBlockWithSigningPeer(Seq(peer1))) { block =>
      BlockAcceptanceLogic
        .processSignatures[IO, SHIBlock](block, mkContext(Balance(250_000L)))
        .value
  ...
  test("reject block with signers without collateral") { implicit sc =>
    forall(dagBlockWithSigningPeer(Seq(peer1))) { block =>
      BlockAcceptanceLogic
        .processSignatures[IO, SHIBlock](block, mkContext(Balance(249_999L)))
        .value
  ...
  test("accept block with signers without balance when collateral is 0") { implicit sc =>
    forall(dagBlockWithSigningPeer(Seq(peer1))) { block =>
      BlockAcceptanceLogic
        .processSignatures[IO, SHIBlock](block, mkContext(Balance(250_000L), Amount(0L)))
        .value
        .map(expect.same(_, Right(())))
    }
  }

  def dagBlockWithSigningPeer(peers: Seq[PeerId]): Gen[Signed[SHIBlock]] =
    signedSHIBlockGen.map(_.copy(proofs = buildProofs(peers)))
