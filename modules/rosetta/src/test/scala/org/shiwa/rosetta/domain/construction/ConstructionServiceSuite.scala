...
import org.shiwa.rosetta.domain.amount.{Amount, AmountValue, AmountValuePredicate}
...
import org.shiwa.rosetta.domain.generators._
import org.shiwa.rosetta.domain.operation.OperationType.Transfer
import org.shiwa.rosetta.domain.operation.{Operation, OperationIdentifier, OperationIndex}
import org.shiwa.schema.address.Address
import org.shiwa.schema.generators.{addressGen, transactionSaltGen}
...
object ConstructionServiceSuite extends MutableIOSuite with Checkers with TransactionGenerator {
  ...
  def generateTestTransactions(wantSignedTransaction: Boolean)(implicit S: SecurityProvider[IO], K: KryoSerializer[IO]) = {
    val getSignedTransaction = (transaction: Hashed[SHITransaction]) => transaction.signed
    val getUnsignedTransaction = (transaction: Hashed[SHITransaction]) => transaction.signed.value

    def txs[A: Encoder](transactionConverter: Hashed[SHITransaction] => A) =
  ...
  def testParseTransactions(
    ...
    testCaseCallbackHandler: (EitherT[F, ConstructionError, ConstructionParse.ParseResult], Hashed[SHITransaction]) => F[Expectations]
  ...
  test("derives public key") { res =>
    ...
    val expected = AccountIdentifier(Address("SHI8Q4CnZ1fSMn1Hrui9MmPogEp5UoT5MSH1LwHg"), None)
  ...
  test("returns the accountIdentifiers for negative operations") { res =>
    ...
    forall(addressGen) { address =>
      val operation = Operation(
        ...
        Amount(AmountValue(-1L), currency.Currency(currency.CurrencySymbol("SHI"), currency.CurrencyDecimal(8L)))
      )
      ...
      .flatMap(_.traverse {
        case (hex, _, _) =>
          cs.parseTransaction(hex, true)
            .rethrowT
            .map(result => expect.eql(Amount(AmountValue(-1L), currency.SHI), result.operations.head.amount))
      })
      .map(_.fold)
  }
  ...
  test("parse signed transaction returns one positive and one negative operation only") {
    val testCase = (parseResult: EitherT[F, ConstructionError, ConstructionParse.ParseResult], _: Hashed[SHITransaction]) =>
  ...
  test("parse signed transaction returns a negative operation that takes from source address") {
    val testCase = (parseResult: EitherT[F, ConstructionError, ConstructionParse.ParseResult], transaction: Hashed[SHITransaction]) =>
      parseResult.rethrowT
  ...
  test("parse signed transaction returns a positive operation with a positive amount") {
    val testCase = (parseResult: EitherT[F, ConstructionError, ConstructionParse.ParseResult], _: Hashed[SHITransaction]) =>
      parseResult.rethrowT
        .map(result => expect.eql(Amount(AmountValue(1L), currency.SHI), result.operations.tail.head.amount))
