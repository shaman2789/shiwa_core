...
object TransactionStorageSuite extends SimpleIOSuite with TransactionGenerator {
  ...
  type TestResources = (
    ...
    MapRef[IO, Address, Option[NonEmptySet[Hashed[SHITransaction]]]],
    KeyPair,
    Address,
    KeyPair,
    Address,
    ...
    SecurityProvider.forAsync[IO].flatMap { implicit sp =>
      KryoSerializer.forAsync[IO](Main.kryoRegistrar ++ sdkKryoRegistrar).flatMap { implicit kp =>
        for {
          ...
          waitingTransactions <- MapRef.ofConcurrentHashMap[IO, Address, NonEmptySet[Hashed[SHITransaction]]]().asResource
          transactionStorage = new TransactionStorage[IO, SHITransaction](lastAccepted, waitingTransactions)
